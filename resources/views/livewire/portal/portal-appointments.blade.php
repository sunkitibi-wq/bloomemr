<div class="min-h-screen bg-slate-50 dark:bg-zinc-950 flex flex-col md:flex-row">
    ...
    <nav class="w-full md:w-64 bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800 flex flex-col p-6 shrink-0">
        ...
        <div class="flex-1 space-y-1">
            ...
            <a class="flex items-center gap-3 px-4 py-2.5 text-trust-navy dark:text-zinc-100 font-semibold bg-slate-100 dark:bg-zinc-800 rounded-lg" href="{{ route('portal.appointments') }}">
        ...
        <header class="h-20 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex justify-between items-center px-8">
            <div>
                <h1 class="font-headline-md font-bold text-trust-navy dark:text-zinc-100 text-lg">
                    {{ __('Scheduling & Appointments') }}
                </h1>
                <p class="text-xs text-zinc-400">
                    {{ __('Request and view patient consultations') }}
                </p>
            </div>
            ...
            @if ($patient)
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-headline-md text-trust-navy dark:text-zinc-100 text-base font-semibold">
                        {{ __('All Appointments for :name', ['name' => $patient->full_name]) }}
                    </h3>
                    <flux:button variant="primary" icon="calendar" wire:click="openRequestForm">
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-xs space-y-4">
                    @forelse ($appointments as $appt)
                        <div class="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800/20 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-lg bg-slate-100 dark:bg-zinc-800 flex flex-col items-center justify-center text-trust-navy dark:text-zinc-300">
                                    <span class="text-[9px] font-bold uppercase">{{ $appt->appointment_date->format('M') }}</span>
                                    <span class="text-lg font-bold leading-tight">{{ $appt->appointment_date->format('d') }}</span>
                                </div>
                                <div>
                                    <p class="text-xs text-zinc-400 font-medium">
                                        {{ $appt->appointment_date->format('l') }}, {{ \Carbon\Carbon::parse($appt->start_time)->format('g:i A') }} - {{ \Carbon\Carbon::parse($appt->end_time)->format('g:i A') }}
                                    </p>
                                    <p class="text-sm font-semibold text-trust-navy dark:text-zinc-100">
                                        {{ __('Consultation Session with :provider', ['provider' => $appt->provider->name]) }}
                                    </p>
                                    @if ($appt->notes)
                                        <p class="text-xs text-zinc-400 mt-1 italic">"{{ $appt->notes }}"</p>
                                    @endif
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
                    ...
                    @empty
                        <p class="text-sm text-zinc-400 py-8 italic text-center">{{ __('No appointments scheduled yet.') }}</p>
                    @endforelse
                </div>
            @else
    ...
    <flux:modal name="portal-appointment-modal" wire:model="isFormOpen" class="max-w-md">
        <form wire:submit="save" class="space-y-4">
            <div>
                <flux:heading size="lg">{{ __('Schedule Appointment') }}</flux:heading>
                <flux:subheading>{{ __('Configure date, time, and notes for the session') }}</flux:subheading>
            </div>

            <flux:select wire:model="providerId" :label="__('Preferred Clinician')" required>
            ...
            <flux:textarea wire:model="notes" :label="__('Reason for Appointment / Notes')" rows="3" placeholder="{{ __('Please outline your main concerns or reasons for scheduling...') }}" />
            <flux:error name="notes" />

            <div class="flex justify-end space-x-2 pt-4">
                <flux:button variant="ghost" wire:click="$set('isFormOpen', false)">{{ __('Cancel') }}</flux:button>
                <flux:button variant="primary" type="submit">{{ __('Book Slot') }}</flux:button>
            </div>
        </form>
    </flux:modal>
</div>
