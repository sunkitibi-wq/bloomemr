    ...
    <nav class="w-full md:w-64 shrink-0 flex flex-col p-5 bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800">
        ...
        <div class="flex items-center gap-3 mb-8 px-1">
            @if ($patient && $patient->photo_path)
                <img src="{{ Storage::url($patient->photo_path) }}" alt="" class="h-10 w-10 rounded-full object-cover border border-slate-200 shadow-sm" />
            @else
                <div class="h-10 w-10 rounded-full bg-trust-navy text-white flex items-center justify-center font-bold text-sm font-mono">
                    {{ $patient ? strtoupper(substr($patient->first_name, 0, 1) . substr($patient->last_name, 0, 1)) : 'P' }}
                </div>
            @endif
            <div>
                <h2 class="font-headline-md text-trust-navy dark:text-zinc-100 font-bold text-[16px]">{{ $patient ? $patient->first_name . "'s Care" : __('Patient Portal') }}</h2>
                <p class="text-zinc-400 text-xs">{{ __('Care Team Active') }}</p>
            </div>
        </div>

        <!-- Navigation Links -->
        <div class="flex-1 space-y-1">
            <a class="flex items-center gap-3 px-4 py-2.5 text-trust-navy dark:text-zinc-100 font-semibold bg-slate-100 dark:bg-zinc-800 rounded-lg" href="{{ route('portal.dashboard') }}">
